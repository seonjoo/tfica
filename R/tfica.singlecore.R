#' tfica with singlecore computation to save time
#'
...
#' @param maxknots (=10) maximum number of knot points for nonparametric spectral density estimation
#' @import coloredICA
#' @import polspline
#' @import dplR
#' @import seewave
...
#  fourier_factor <- (4 * pi)/(k0 + sqrt(2 + k0^2))
  X.morlet = lapply(1:nrow(Xc),function(idx){morlet(y1 = Xc[idx,], x1 = 1:T, dj = sopoc, siglvl = 0.95)})
  ...
  lspec.ests = do.call(rbind,
                       lapply(1:nrow(S.mat),function(jjj){
                         obj=S.mat[jjj,]
                         mm = length(obj)
                         tmpfit<-c()
